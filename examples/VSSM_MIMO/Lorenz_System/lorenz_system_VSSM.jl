...
for i=1:LONG-1 #i counts the cycles of the simulation
  ...
  u[i]=x_p_des[i]-σₐ*(y[i]-x[i])
  # Get the system "reaction" (the exact model of the system)
  x_p[i]=σₑ*(y[i]-x[i])+u[i]
  y_p[i]=x[i]*(ρₑ-z[i])-y[i]
  z_p[i]=x[i]*y[i]-βₑ*z[i]
...
ylabel(L"q \enspace [m]")
plot(t[1:LONG-1],xN[1:LONG-1],color="#7684FF")
#plot(t[3:LONG-1],yN[3:LONG-1],color="#2A40FF")
#plot(t[3:LONG-1],zN[3:LONG-1],color="#3B427F")

plot(t[1:LONG-1],x[1:LONG-1],color="#FFAA41","r--")
...
xlabel(L"Time \enspace [s]")
ylabel(L"error \enspace [m]")
plot(t[1:LONG-1],xN[1:LONG-1]-x[1:LONG-1],color="red")

figure(3)
grid("on")
title("Phase Space of Error Metric (S_p vs S)")
plot(S_x[1:LONG-1],S_p_x[1:LONG-1],color="blue")
